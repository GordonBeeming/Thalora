...
test.describe('URL Shortening', () => {
  ...
  test('should handle invalid URLs', async ({ page }) => {
    ...
    await page.fill('input[placeholder="Enter your URL here (e.g., gordonbeeming.com)"]', 'invalid-url');
    await page.click('button:has-text("Shorten URL")');
    await expect(page.getByText('Invalid URL format')).toBeVisible();

    // Test HTTP URL (should require HTTPS)
    await page.fill('input[placeholder="Enter your URL here (e.g., gordonbeeming.com)"]', 'http://example.com');
    await page.click('button:has-text("Shorten URL")');
  ...
  test('should create multiple URLs', async ({ page }) => {
    ...
    for (let i = 0; i < urls.length; i++) {
      // Create shortened URL
      await page.fill('input[placeholder="Enter your URL here (e.g., gordonbeeming.com)"]', urls[i]);
      await page.click('button:has-text("Shorten URL")');
  ...
  test('should handle very long URLs', async ({ page }) => {
    ...
    await page.fill('input[placeholder="Enter your URL here (e.g., gordonbeeming.com)"]', longUrl);
    await page.click('button:has-text("Shorten URL")');
